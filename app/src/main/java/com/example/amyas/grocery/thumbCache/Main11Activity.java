package com.example.amyas.grocery.thumbCache;
...
public class Main11Activity extends AppCompatActivity {
    ...
    private static final String[] bitmapUrl = new String[]{
    ...
    private class MyAdapter extends RecyclerView.Adapter<MyAdapter.MyHolder>{
        ...
        private Set<BitmapTask> mBitmapTasks;
        ...
        public MyAdapter() {
            ...
            mBitmapTasks = new HashSet<>();
            mRecyclerView.addOnScrollListener(new RecyclerView.OnScrollListener() {
                @Override
                public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
                    Log.e(TAG, "onScrollStateChanged: ");
                    if (newState==RecyclerView.SCROLL_STATE_IDLE){
                        loadTask(currentIndex, visibleCount);
                    }else {
                        cancelTasks(false);
                    }
                }
...
//                    visibleCount = visibleItemCount;
                    Log.e(TAG, "MyAdapter: currentIndex，visibleCount = "+currentIndex+","+visibleCount);
                    if (visibleCount>0||isFirstLoad){
                        isFirstLoad = false;
                        loadTask(currentIndex, visibleCount);
                    }
                }
        ...
        public void onBindViewHolder(MyHolder holder, int position) {
            String url = bitmapUrl[position];
            holder.onBind(url);
        }

        @Override
        public int getItemCount() {
            return bitmapUrl.length;
        }

        private void cancelTasks(boolean mayInterruptIfRunning){
            for (BitmapTask task : mBitmapTasks) {
                task.cancel(mayInterruptIfRunning);
            }
        }

        /**
         * 查看 currentIndex+visibleCount 是否会变化
         * @param currentIndex
         * @param visibleCount
         */
        private void loadTask(int currentIndex, int visibleCount){
            for (int i = currentIndex; i < currentIndex+visibleCount; i++) {
                String s = bitmapUrl[i];
                Bitmap bitmap = mLruCache.get(s);
                if (bitmap!=null){
                    ImageView imageView = mRecyclerView.findViewWithTag(s);
                    assert imageView!=null;
                    imageView.setImageBitmap(bitmap);
                }else {
                    BitmapTask task = new BitmapTask();
                    task.execute(s);
                    mBitmapTasks.add(task);
                }
